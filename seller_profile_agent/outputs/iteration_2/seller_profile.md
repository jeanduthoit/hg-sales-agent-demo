...
| Field              | Value                          |
| ------------------ | ------------------------------ |
| Product line sold  | Amazon AppFlow                 |
| HG SKU             | `Amazon AppFlow` (id: `28703`) |
| HG category        | `None`                         |
| Intent topic (HG)  | `None`                         |
...
| ------------------------- | ----------------------------- | ------------------------------------------------------------ |
| Technology Category Need  | `company_technographic`       | Max `intensity` in `**None`**                                |
| Purchase Intent Signal    | `company_intent`              | Topic `**None**` → `topics[].score`, `topics[].last_seen_at` |
...
- Query product: `Amazon AppFlow`
- Status: `resolved`
- Competitor products extracted: **2**
...
- **min_revenue_usd** >= 2000000 — HG field: `company_firmographic.revenue`. Threshold entered by the sales user when creating the seller profile.
- **min_employees** >= 20 — HG field: `company_firmographic.employeeCount`. Threshold entered by the sales user when creating the seller profile.
- **min_it_spend_usd** >= 100000 — HG field: `company_spend.totalSpendAmount (Total IT)`. Threshold entered by the sales user when creating the seller profile.
- **category_adjacency_search** technographic_signal HG technographic product — HG field: `company_technographic.categories / search_companies`. Prospects should show activity in the same HG category as the sold product to avoid scoring random accounts with zero category fit.
- **exclude_target_product_installed** excludeTechnologies Amazon AppFlow — HG field: `search_companies.excludeTechnologies`. Land motion only: exclude accounts where the seller HG SKU is already installed.
...
1. **Product binding** — SKU `Amazon AppFlow` and category `None` define what “installed” and “need” mean.
2. **Intent topic** — `None` must exist in HG; if wrong, Purchase Intent collapses to 0 with 0% reliability.
